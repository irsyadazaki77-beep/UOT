(() => {
    ...
    const categories = new Set(["all", "programming", "database", "design", "analytics", "web", "cyber", "ai", "cloud", "mobile", "quantum", "blockchain", "biotech", "futuretech"]);
    ...
    function readSession() {
        const raw = read(sessionStore, SESSION_KEY, null);
        const result = validatePayload(raw);
        if (!result.ok) {
            if (raw) quarantine(sessionStore, SESSION_KEY, raw);
            return result;
        }
        if (JSON.stringify(raw) !== JSON.stringify(result.value)) write(sessionStore, SESSION_KEY, result.value);
        return result;
    }
    ...
    function migrate() {
        try {
            const current = Number(localStore?.getItem(MIGRATION_KEY) || 0);
            if (current >= SCHEMA_VERSION) return;
            const bookmarks = sanitizeBookmarks(read(localStore, "eduquestBookmarks", []));
            const lmsProgress = sanitizeLmsProgress(read(localStore, "eduquestLmsProgress", {}));
            write(localStore, "eduquestBookmarks", bookmarks);
            write(localStore, "eduquestLmsProgress", lmsProgress);
            const best = Math.round(clamp(localStore?.getItem("eduquestBestScore"), 0, 100, 0));
            localStore?.setItem("eduquestBestScore", String(best));
            localStore?.setItem(MIGRATION_KEY, String(SCHEMA_VERSION));
        } catch (error) {
